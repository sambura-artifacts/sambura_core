...
class DownloadDockerBlobUseCase {
  ...
  final Logger _log = LoggerConfig.getLogger('DownloadDockerBlobUseCase');

  DownloadDockerBlobUseCase(
    this._httpClient,
  ...
  Future<Stream<List<int>>> executeProxyStream({
    ...
    final lockKey = 'lock:docker-blob:${input.fileName.hashCode}';
    ...
    if (!acquired) {
      ...
      _log.info(
        '⏳ Aguardando download paralelo Docker Blob: ${input.fileName}',
      );
    ...
    try {
      _log.info('🌐 Mirroring Docker Blob: $remoteUrl');
      final response = await _httpClient.stream(Uri.parse(remoteUrl));
      _metrics.observeHistogram(
        'sambura_docker_proxy_resolution_duration_ms',
      ...
      unawaited(
        _createArtifact
            ...
            .then((_) {
              ...
              _log.info('✅ Docker Blob Mirroring concluído: ${input.fileName}');
            })
            .catchError((e) {
              ...
              _log.severe('❌ Falha ao espelhar Docker Blob: $e');
            })
            .whenComplete(() {
      ...
      _metrics.incrementCounter('sambura_docker_proxy_errors_total');
      _log.severe('❌ Erro no Proxy Docker: $e');
      rethrow;
    }
  }
}
