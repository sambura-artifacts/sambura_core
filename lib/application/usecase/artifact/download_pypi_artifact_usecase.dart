...
class DownloadPypiArtifactUseCase {
  ...
  final Logger _log = LoggerConfig.getLogger('DownloadPypiArtifactUseCase');

  DownloadPypiArtifactUseCase(
    this._httpClient,
  ...
  Future<Stream<List<int>>> executeProxyStream({
    ...
    final lockKey =
        'lock:pypi:${input.packageName}:${input.version}:${input.fileName.hashCode}';

    // 1. Controle de Concorrência
    final acquired = await _cache.acquireLock(lockKey);
    if (!acquired) {
      _metrics.incrementCounter('sambura_download_concurrency_waits_total');
      _log.info('⏳ Aguardando download paralelo PyPI: ${input.fileName}');
    ...
    try {
      ...
      unawaited(
        _createArtifact
            ...
            .then((_) {
              ...
              _log.info('✅ PyPI Mirroring concluído: ${input.fileName}');
            })
            .catchError((e) {
              ...
              _log.severe('❌ Falha ao espelhar PyPI: $e');
            })
            .whenComplete(() {
      ...
      _metrics.incrementCounter('sambura_pypi_proxy_errors_total');
      _log.severe('❌ Erro no Proxy PyPI: $e');
      rethrow;
    }
  }
}
